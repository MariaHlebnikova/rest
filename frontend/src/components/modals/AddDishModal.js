...
const AddDishModal = ({ show, onHide, orderId, onDishAdded }) => {
  ...
  const handleAddDishes = async () => {
    ...
    try {
      ...
      // Добавляем каждое блюдо по отдельности
      const addedDishes = [];
      for (const [dishId, quantity] of Object.entries(cart)) {
        if (quantity > 0) {
          const response = await api.post(`/orders/${orderId}/add-item`, {
            dish_id: parseInt(dishId),
            quantity: quantity
          });
          addedDishes.push(response.data);
        }
      }
      
      setSuccess(`Успешно добавлено ${Object.keys(cart).length} блюд в заказ!`);
      
      setTimeout(() => {
        if (onDishAdded) {
          onDishAdded({
            message: `Добавлено ${Object.keys(cart).length} блюд`,
      ...
      console.error('Ошибка добавления блюд:', err);
      setError(err.response?.data?.error || 'Ошибка добавления блюд');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Modal show={show} onHide={onHide} size="xl" scrollable>
      <Modal.Header closeButton>
        <Modal.Title>
          Добавление блюд в заказ #{orderId}
        </Modal.Title>
      </Modal.Header>
      <Modal.Body>
        ...
        {loading && !dishes.length ? (
          ...
          <div className="row">
            {/* Левая колонка - категории и меню */}
                  ...
                  {cartItems.length === 0 ? (
                    ...
                    <>
                      ...
                      <div className="border-top pt-3">
                        ...
                        >
                          {loading ? 'Добавление...' : 'Добавить в заказ'}
                        </Button>
                      </div>
                    </>
                  )}
